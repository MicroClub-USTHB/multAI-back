...
def mock_staged_upload_storage():
    mock = AsyncMock()
    mock.store_staging_object.return_value = StoredObject(
        storage_key="test_storage_key", content_type="image/jpeg", file_name="photo.jpg"
    )
    mock.create_presigned_staging_upload.return_value = (
        "staging/upload-requests/req1/photo1.jpg",
...
def _make_photo(photo_id, request_id, **overrides):
    defaults = dict(
        ...
        size_bytes=1000,
        staging_storage_key="staging/x.jpg",
...
async def test_create_direct_group_sets_source_and_completed_processing(
    ...
    event_id = uuid.uuid4()
    group_id = uuid.uuid4()
    mock_upload_request_group_querier.create_upload_request_group.return_value = (
        _make_group(
            group_id,
            event_id,
            mock_staff_user.id,
...
async def test_register_direct_batch_creates_pending_photos_and_returns_urls(
    ...
    mock_upload_request_group_querier.get_upload_request_group_by_id.return_value = (
        _make_group(
            group_id,
            event_id,
            mock_staff_user.id,
    ...
    mock_upload_request_photo_querier.create_direct_upload_request_photo.return_value = _make_photo(
        photo_id,
        request_id,
        staging_storage_key="staging/upload-requests/req1/photo1.jpg",
    )
...
async def test_confirm_direct_upload_success_updates_transfer_status(
    ...
    mock_upload_request_photo_querier.get_upload_request_photo_by_id.return_value = (
        existing_photo
    )
    mock_staged_upload_storage.stat_staging_object.return_value = ObjectStat(
        size=1000, content_type="image/jpeg"
    )
...
async def test_confirm_direct_upload_marks_failed_when_object_missing(
    ...
    mock_upload_request_photo_querier.fail_upload_request_photo_transfer.assert_awaited_once_with(
        id=photo_id
    )
...
async def test_approve_request_blocked_when_photo_not_fully_uploaded(
    ...
    not_uploaded_photo = _make_photo(
        photo_id, request_id, transfer_status="pending_upload"
    )
...
async def test_resume_direct_group_reissues_urls_for_pending_and_failed_only(
    ...
    async def _requests_iter(group_id):
        yield _make_request(
            request_id, event_id, mock_staff_user.id, group_id, photo_count=2
        )

    mock_upload_request_querier.list_upload_requests_by_group_id = _requests_iter

    failed_photo = _make_photo(
        failed_photo_id,
        request_id,
        file_name="fail.jpg",
        staging_storage_key="staging/fail.jpg",
        transfer_status="failed",
    )
    uploaded_photo = _make_photo(
        uploaded_photo_id,
        request_id,
        file_name="ok.jpg",
        staging_storage_key="staging/ok.jpg",
        transfer_status="uploaded",
    )
    ...
    mock_staged_upload_storage.create_presigned_staging_upload.return_value = (
        "staging/fail.jpg",
        "https://minio.local/resumed",
    )
...
async def test_approve_request_publishes_drive_sync_event_for_direct_photo_only(
    ...
    async def _photos_iter(upload_request_id):
        yield _make_photo(
            photo_id, request_id, transfer_status="uploaded", source="direct"
        )
