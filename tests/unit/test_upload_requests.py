...
async def test_create_request_success(
    ...
    mock_upload_request_photo_querier.create_upload_request_photo.return_value = (
        UploadRequestPhoto(
            id=uuid.uuid4(),
    ...
    with (
        patch(
            "app.service.upload_requests.GoogleDriveClient.download_file",
...
async def test_create_request_duplicate_conflict(
    ...
    mock_upload_request_photo_querier.create_upload_request_photo.side_effect = (
        create_integrity_error("23505")
    )
    ...
    with patch(
        ...
        with pytest.raises(HTTPException) as exc:
            await upload_requests_service.create_request(
                event_id=event_id, photos=photos, requested_by=mock_staff_user
            )
        ...
        mock_staged_upload_storage.delete_storage_key.assert_called_once_with(
            "test_storage_key"
        )
...
async def test_create_group_from_folder(
    ...
    with patch("app.service.upload_requests.NatsClient.js_publish") as mock_publish:
        ...
        mock_publish.assert_called_once()
        assert (
            mock_publish.call_args[0][0]
            == NatsSubjects.STAFF_UPLOAD_GROUP_IMPORT_REQUESTED
        )
...
async def test_process_group_import_no_images(
    ...
    mock_staff_drive_service.staff_user_querier.get_staff_user_by_id.return_value = (
        mock_staff_user
    )

    # Return 0 images
    with patch(
        "app.service.upload_requests.GoogleDriveClient.list_folder_files",
        return_value=[],
    ):
        ...
        kwargs = mock_upload_request_group_querier.fail_upload_request_group_processing.call_args[
            0
        ][0]
...
async def test_approve_request_without_side_effects(
    ...
    (
        ...
        final_keys,
        created_photos,
    ) = await upload_requests_service._approve_request_without_side_effects(
