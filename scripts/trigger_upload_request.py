...
async def main():
    ...
    async with engine.connect() as conn:
        ...
        row = (
            await conn.execute(sqlalchemy.text("SELECT id FROM staff_users LIMIT 1"))
        ).fetchone()
        staff_user_id = row[0]
        ...
        # Need an event
        event_row = (
            await conn.execute(sqlalchemy.text("SELECT id FROM events LIMIT 1"))
        ).fetchone()
        if not event_row:
            ...
            await conn.execute(
                sqlalchemy.text(
                    "INSERT INTO events (id, title, date, location) VALUES (:id, 'Test Event', now(), 'Test Location')"
                ),
                {"id": ev_id},
            )
            event_id = ev_id
        ...
        upload_service = UploadRequestsService(
            ...
            staff_notifications_service=StaffNotificationsService(
                notif_queries.AsyncQuerier(conn)
            ),
        ...
        objects = bucket.client.list_objects(
            bucket.bucket_name, prefix=bucket.file_prefix + "/", recursive=True
        )
        photo_inputs = []
        ...
        await upload_service.approve_request(
            request_id=req_details.id, approved_by=staff_user
        )
